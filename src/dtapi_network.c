	...
	enum tcore_notification_command command, unsigned int data_len, const void *data)
{
	...
	case TNOTI_NETWORK_LOCATION_CELLINFO: {
		...
		if (NETWORK_SERVICE_TYPE_LTE == network_service_type) {
			...
			telephony_network_emit_cell_info_tac(network, location->lac);
		} else {
			telephony_network_set_lac(network, location->lac);
			telephony_network_set_tac(network, -1);
			telephony_network_emit_cell_info_lac(network, location->lac);
		}
		...
		telephony_network_emit_cell_info(network,
			location->lac, location->cell_id);
		telephony_network_emit_cell_info_cell_id(network,
			location->cell_id);
	}
	break;
	...
	case TNOTI_NETWORK_LOCATION_CELLINFO_CDMA: {
		...
		/* Emit signal */
		telephony_network_emit_cell_info_system_id(network, location->system_id);
		telephony_network_emit_cell_info_network_id(network, location->network_id);
		telephony_network_emit_cell_info_bs_id(network, location->bs_id);
		telephony_network_emit_cell_info_bs_latitude(network, location->bs_latitude);
		telephony_network_emit_cell_info_bs_longitude(network, location->bs_longitude);
	}
	break;
